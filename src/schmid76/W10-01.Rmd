...
```{r,warning=FALSE,eval=TRUE,message=FALSE}
#plotting function
image.plot<-function(coverage,landsat){
  vector_classes <- cut(coverage@data$COVRG, c(0, 20, 40, 60, 80, 100, 120))
  ...
  yat = seq(extent(landsat)@ymin, 
                  extent(landsat)@ymax, length.out = 5)
  xat = seq(extent(landsat)@xmin, 
                  extent(landsat)@xmax, length.out = 5)
